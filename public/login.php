...
$return = isset($_GET['return']) ? $_GET['return'] : 'avaleht.php';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $email = mysqli_real_escape_string($yhendus, $_POST['email']);
    $password = $_POST['password'];

    $stmt = mysqli_prepare($yhendus, "SELECT id, password_hash, role, first_name, last_name FROM users WHERE email = ? LIMIT 1");
    mysqli_stmt_bind_param($stmt, 's', $email);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $id, $hash, $role, $first_name, $last_name);
